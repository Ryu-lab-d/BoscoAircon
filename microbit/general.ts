// ============================================================================
// Nexus aircon monitor — GENERAL (regular homeroom) classroom firmware.
//
// One of the 11 regular rooms: CC7A-CC7D, CC8A-CC8D, CC9A-CC9C. Unlike a
// special room, a regular room has hardcoded default allowed hours
// (08:00-11:40, 12:30-16:20) that apply whenever no override slot covers the
// current time — the app only sends override windows (e.g. a vacate window
// while the class is at PE/Project/Art/Computer elsewhere), not the whole day.
//
// Edit ROOM_ID (and the Wi-Fi/Telegram constants below) to this unit's own
// values before flashing. Everything else is identical across all 11 units.
// ============================================================================
...
// Hardcoded default allowed hours, used whenever no override slot covers the
// current time (including on a weekday the app never sent any WD entry for).
const DEFAULT_HOURS: TimeSlot[] = [
...
//   <RoomId>|[WD<n>|<slot>...]*|E|<date>|<slot>...|H|<range>...|T|<chatId>...
// A regular room only gets a WD<n> marker for a day that has at least one
// override window; other days are simply absent, meaning "use DEFAULT_HOURS
// all day". WD<n>, "E", "H", "T" are section markers; any section may be
// omitted entirely if it has nothing to say.
// ============================================================================
...
function isWithinDefaultHours(nowHHMM: string): boolean {
    for (let i = 0; i < DEFAULT_HOURS.length; i++) {
        let slot = DEFAULT_HOURS[i]
...
// Holiday, then today's event override, then today's override slots (if any),
// then the hardcoded default hours — first match wins. Checking override
// slots directly (rather than only when the day has "no WD entry at all")
// means a single vacate window correctly punches a hole in the default
// schedule without turning the rest of that day off too.
function shouldBeOn(chart: Chart, weekDay: number, nowHHMM: string, todayStr: string): boolean {
    ...
    return isWithinDefaultHours(nowHHMM)
}
